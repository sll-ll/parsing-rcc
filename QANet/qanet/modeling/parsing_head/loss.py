...
class ParsingLossComputation(object):
    ...
    def __call__(self, logits, parsing_targets):
        ...
        acc = torch.eq(parsing_logits.argmax(dim=1), parsing_targets).sum() / (parsing_targets.shape[0] * parsing_targets.shape[1] * parsing_targets.shape[2])
        ...
        return parsing_loss, parsingiou_targets, acc
